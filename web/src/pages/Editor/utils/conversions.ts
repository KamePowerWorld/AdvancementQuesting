...
/** デフォルトアイテム */
const DEFAULT_ITEM = NamespacedId.parse('minecraft:stone')

/**
 * JSON境界の読み取り: fullId 文字列を NamespacedId にする。
 * 旧データに省略形が残っている場合に備え parseUserInput で読む（書き込みは常に完全形式）。
 */
function readId(id: string | null | undefined, fallback: NamespacedId = DEFAULT_ITEM): NamespacedId {
  return id ? NamespacedId.parseUserInput(id) : fallback
}

/** JSON境界の読み取り (省略可能フィールド用): 空なら undefined */
function readIdOrUndefined(id: string | null | undefined): NamespacedId | undefined {
  return id ? NamespacedId.parseUserInput(id) : undefined
}
...
export function questToNode(q: Quest): EditorNode {
  ...
  return {
    ...
    tasks: (q.conditions ?? []).map((c, i) => ({
      ...
      ...(c.type === 'advancement' ? { advancementId: readIdOrUndefined(c.advancementId) } : {}),
      ...(c.type === 'item' ? { itemType: readId(c.itemType), count: c.count ?? 1, ...(c.nbt ? { nbt: c.nbt } : {}), ...(c.displayName ? { displayName: c.displayName } : {}) } : {}),
      ...(c.type === 'delivery' ? { itemType: readId(c.itemType), count: c.count ?? 1, ...(c.nbt ? { nbt: c.nbt } : {}), ...(c.displayName ? { displayName: c.displayName } : {}) } : {}),
      ...(c.type === 'stat' ? { statType: c.statType ?? '', statId: readIdOrUndefined(c.statId), count: c.count ?? 1 } : {}),
    ...
    rewards: (q.rewards ?? []).map((r, i) => {
      ...
      if (r.type === 'item') return { ...base, type: 'item', itemType: readId(r.itemId), count: r.count ?? 1, ...(r.nbt ? { nbt: r.nbt } : {}), ...(r.displayName ? { displayName: r.displayName } : {}) }
...
export function nodeToApiBody(node: EditorNode, edgeList: EditorEdge[]) {
  const conditions: Condition[] = (node.tasks ?? []).map((t) => {
    if (t.type === 'advancement') return { id: t.id, type: 'advancement' as const, advancementId: t.advancementId?.toString() ?? '' }
    if (t.type === 'item') return { id: t.id, type: 'item' as const, itemType: (t.itemType ?? DEFAULT_ITEM).toString(), count: t.count ?? 1, ...(t.nbt ? { nbt: t.nbt } : {}), ...(t.displayName ? { displayName: t.displayName } : {}) }
    if (t.type === 'delivery') return { id: t.id, type: 'delivery' as const, itemType: (t.itemType ?? DEFAULT_ITEM).toString(), count: t.count ?? 1, ...(t.nbt ? { nbt: t.nbt } : {}), ...(t.displayName ? { displayName: t.displayName } : {}) }
    if (t.type === 'checkmark') return { id: t.id, type: 'checkmark' as const, label: t.value ?? '' }
    if (t.type === 'stat') return { id: t.id, type: 'stat' as const, statType: t.statType ?? '', statId: t.statId?.toString() ?? '', count: t.count ?? 1 }
  ...
  const rewards: Reward[] = (node.rewards ?? []).map((r) => {
    if (r.type === 'item') return { type: 'item' as const, itemId: (r.itemType ?? DEFAULT_ITEM).toString(), count: r.count ?? 1, ...(r.nbt ? { nbt: r.nbt } : {}), ...(r.displayName ? { displayName: r.displayName } : {}) }
...
export function proposalToNode(p: Proposal, localEdit?: EditorNode): ProposalNode {
  ...
  const base: ProposalNode = {
    ...
    icon: readId(snap.icon), title: snap.title ?? '提案', subtitle: snap.subtitle ?? '',
